...
const TransactionsPage: React.FC = () => {
  ...
  const {
    data: dataChart,
    isFetched: isFetchedChart,
    refetch: refetchChart,
  } = useGetAPI(`/api/orderList/chartData`, "ordersChart", config);
  const displayCard = () => {
    if (data && isFetched) return data.map((orders: any, index: number) => <OrderCard key={index} order={orders} />);
    else {
      refetch();
    }
  };
    ...
    <ProtectedRouteTenant>
      <div>
        ...
        <Tabs defaultValue="userOrders" className="w-[900px]  ">
          <TabsList>
            <TabsTrigger value="userOrders" className="w-[300px] p-2 ">
              Order
            </TabsTrigger>
            <TabsTrigger value="reportSales" className="w-[300px] p-2 ">
              Sales
            </TabsTrigger>
            <TabsTrigger value="occupancy" className="w-[300px] p-2 ">
              Occupancy
            </TabsTrigger>
          </TabsList>
          <TabsContent value="userOrders">
            <div className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[70%]">{displayCard()}</div>
          </TabsContent>
