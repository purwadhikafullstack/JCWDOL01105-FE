...
const OccupancyCard: React.FC<CardProps> = ({ property }: any) => {
  const displayCard = (data: any) => {
    return data.map((rooms: any, index: number) => (
      <div>
        <CardContent key={index}>{rooms.room_name}</CardContent>
        <CardContent>Availability : {rooms.availability}</CardContent>
      </div>
    ));
  };
  return (
    <Card>
      ...
      <CardFooter className="gridcol-3 gap-2"></CardFooter>
    </Card>
  );
};
